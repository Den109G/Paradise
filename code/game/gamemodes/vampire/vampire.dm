...
/datum/game_mode/proc/forge_vampire_objectives(var/datum/mind/vampire)
	...
	if(prob(20))
		...
		else if(!(locate(/datum/objective/survive) in vampire.objectives))
...
Вы уязвимы перед святостью, звёздным светом и огнём. Не выходите в космос, избегайте священника, церкви и, особенно, святой воды."} //russian text, HSSSSS
...
/datum/vampire
	...
	var/list/upgrade_tiers = list(
		/obj/effect/proc_holder/spell/self/vampire/rejuvenate = 0,
...
/datum/vampire/proc/adjust_nullification(base, extra) //rn is not used
...
/datum/vampire/proc/handle_bloodsucking(mob/living/carbon/human/H, suck_rate = 5 SECONDS)
	...
	owner.visible_message("<span class='danger'>[owner.name] грубо хватает шею [H] и вонзает в неё клыки!</span>", "<span class='danger'>Вы вонзаете клыки в шею [H] и начинаете высасывать [genderize_ru(H.gender, "его", "её", "его", "их")] кровь.</span>", "<span class='notice'>Вы слышите тихий звук прокола и влажные хлюпающие звуки.</span>")
	if(!iscarbon(owner))
		H.LAssailant = null
	else
		H.LAssailant = owner
...
=======*/
		...
		if(!H.ckey && !H.player_ghosted)//Only runs if there is no ckey and the body has not being ghosted while alive
			to_chat(owner, "<span class='notice'><b>Вы накопили [bloodtotal] единиц[declension_ru(bloodtotal, "у", "ы", "")] крови[bloodusable != bloodusable ? ", и теперь вам доступно [bloodusable] единиц[declension_ru(bloodusable, "а", "ы", "")] крови" : ""].</b></span>")
			owner.set_nutrition(min(NUTRITION_LEVEL_WELL_FED, owner.nutrition + 5))
		else
