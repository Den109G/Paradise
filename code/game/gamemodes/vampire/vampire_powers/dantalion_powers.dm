...
/datum/vampire_passive/increment_thrall_cap/on_apply(datum/vampire/V)
...
/obj/effect/proc_holder/spell/vampire/targetted/enthrall/choose_targets(mob/user = usr)
...
/obj/effect/proc_holder/spell/vampire/targetted/enthrall/cast(list/targets, mob/user = usr)
	var/datum/vampire/vampire = user.mind.has_antag_datum(/datum/vampire)
...
/obj/effect/proc_holder/spell/vampire/targetted/enthrall/proc/can_enthrall(mob/living/user, mob/living/carbon/C)
	...
	var/datum/vampire/V = user.mind.has_antag_datum(/datum/vampire)
	if(V.subclass.thrall_cap <= length(user.mind.som.serv))
		...
		return
	if(ismindshielded(C) || C.mind.has_antag_datum(/datum/vampire) || C.mind.has_antag_datum(/datum/antagonist/mindslave))
...
/obj/effect/proc_holder/spell/vampire/targetted/enthrall/proc/handle_enthrall(mob/living/user, mob/living/carbon/human/H)
	if(!istype(H))
		return FALSE
...
/obj/effect/proc_holder/spell/vampire/thrall_commune/choose_targets(mob/user = usr) // Returns the vampire and their thralls. If user is a thrall then it will look up their master's network
	var/list/mob/living/targets = list()
	var/datum/vampire/V = user.mind.has_antag_datum(/datum/vampire) // if the user is a vampire

	if(!V)
		for(var/datum/mind/M in user.mind.som.masters) // if the user is a thrall
			V = M.has_antag_datum(/datum/vampire)
			if(V)
				break

	if(!V)
		return
	if(!V.owner.mind.som) // I hate som
		stack_trace("Dantalion Thrall datum ended up null.")
		return

	for(var/datum/mind/thrall in V.owner.mind.som.serv)
		targets += thrall.current
	targets += V.owner.mind.current
	return targets

/*/obj/effect/proc_holder/spell/vampire/thrall_commune/proc/create_new_targeting()
