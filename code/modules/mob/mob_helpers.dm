...
/proc/species_clothing(species) //used to determine, which species use what clothings' sprite
	if(!species)
		return SPECIES_HUMAN

	switch(species)
		//golem subtypes
		if(SPECIES_GOLEM_ADAMANTINE)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_PLASMA)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_DIAMOND)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_GOLD)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_SILVER)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_PLASTEEL)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_TITANIUM)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_PLASTITANIUM)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_ALLOY)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_WOOD)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_URANIUM)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_PLASTIC)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_SAND)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_GLASS)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_BLUESPACE)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_BANANIUM)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_TRANQUILLITITE)
			species = SPECIES_GOLEM_BASIC
		if(SPECIES_GOLEM_CLOCKWORK)
			species = SPECIES_GOLEM_BASIC
		//monkey subtypes
		if(SPECIES_FARWA)
			species = SPECIES_MONKEY
		if(SPECIES_WOLPIN)
			species = SPECIES_MONKEY
		if(SPECIES_NEARA)
			species = SPECIES_MONKEY
		if(SPECIES_STOK)
			species = SPECIES_MONKEY
		//shadow subtypes
		if(SPECIES_SHADOWLING)
			species = SPECIES_SHADOW_BASIC
		if(SPECIES_LESSER_SHADOWLING)
			species = SPECIES_SHADOW_BASIC
		//unathi subtypes
		if(SPECIES_ASHWALKER_BASIC)
			species = SPECIES_UNATHI
		if(SPECIES_ASHWALKER_SHAMAN)
			species = SPECIES_UNATHI
		if(SPECIES_DRACONOID)
			species = SPECIES_UNATHI
		/*//vox subtypes
		if(SPECIES_VOX_ARMALIS) //they are diffrent enough to have their own sprites
			species = SPECIES_VOX*/
